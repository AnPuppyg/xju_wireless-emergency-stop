...
namespace SHELL{
    ...
    void shell_init(void){
        ...
        shell.addCommand(F("set_voltage_protect 设置电压保护\n  set_voltage_protect [默认是否开启] [低压值:V]"),[](int argc, char** argv){
            ...
            OTHER_FUNCTION::low_voltage_protect_ctrl(state);
            return 0;
        });
        ...
        shell.addCommand(F("set_voltage_protect 设置电压保护\n  set_voltage_protect [默认是否开启] [低压值:V]"),[](int argc, char** argv){
            ...
            OTHER_FUNCTION::default_high_voltage_protect_state=state;
            OTHER_FUNCTION::high_voltage_protect_value=value;
            OTHER_FUNCTION::high_voltage_protect_ctrl(state);
            return 0;
        });
