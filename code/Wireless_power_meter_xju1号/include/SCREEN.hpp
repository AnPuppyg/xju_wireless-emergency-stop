...
namespace SCREEN {
    ...
    constexpr uint16_t border_color = TFT_GREEN;    //边框颜色
    ...
    void mian_page() {
        ...
        constexpr uint16_t current_color = TFT_BLUE;  // 电流颜色
        constexpr uint16_t power_color = TFT_YELLOW;    // 功率颜色
        constexpr uint16_t line_color = TFT_GREEN; // 分割线颜色
        ...
        // 绘制分割线
        for (int i = 0; i < 3; i++) {
            clk.drawLine(162 + i, 0, 162 + i, 135, TFT_GREEN); // 分割线用黄色
        }
